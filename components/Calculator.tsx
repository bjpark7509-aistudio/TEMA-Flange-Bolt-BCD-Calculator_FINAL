...
export const Calculator: React.FC<Props> = ({ inputs, onInputChange, onOptimize, onResetOptimize, onGlobalReset, onClearRecords, onLoad, results, boltMaterials, plateMaterials, temaBoltData, gasketTypes, ringStandards }) => {
  ...
  const handleManualStart = () => {
    // Commit local values to global state to trigger calculation via onOptimize
    const nextInputs = {
      ...inputs,
      ...localManualValues
    };
    // Notify the parent to update inputs and then trigger optimization logic
    onInputChange(nextInputs, 'manual_start_calculation');
    onOptimize?.(nextInputs);
  ...
  const inputClass = "w-full px-3 py-2 bg-white border border-gray-300 rounded-md focus:ring-2 focus:ring-sky-500 focus:border-transparent transition-all outline-none text-sm font-mono";
      ...
      <div className="space-y-6">
        <section className="space-y-3 bg-red-50 p-4 rounded-lg border border-red-100 shadow-sm">
          ...
          <div className="pt-2">
            ...
            <select name="shellMaterial" value={inputs.shellMaterial} onChange={handleChange} className={`${inputClass} border-sky-200 bg-sky-50/20 font-bold`}>
