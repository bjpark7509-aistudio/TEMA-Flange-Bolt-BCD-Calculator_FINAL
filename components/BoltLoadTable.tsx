...
export const BoltLoadTable: React.FC<Props> = ({ inputs, results, boltMaterials, setBoltMaterials, plateMaterials, setPlateMaterials, temaBoltData, setTemaBoltData, tensioningData, setTensioningData, gasketTypes, setGasketTypes, ringStandards, setRingStandards }) => {
  ...
  // Fix: Removed duplicate declaration of currentBoltRef (previously on line 184 and 724)
            ...
            <section className="lg:col-span-8 space-y-4">
              <div className="h-full bg-sky-50/50 rounded-xl border border-sky-100 p-5">
                <h3 className="text-[11px] font-black text-sky-700 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                  <i className="fa-solid fa-weight-hanging"></i> Bolt Load Calculation
                </h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-white p-5 rounded-xl border border-sky-100 shadow-sm space-y-4 flex flex-col">
          ...
          <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl p-6 flex flex-col animate-in fade-in zoom-in duration-200">
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                ...
                <input type="number" step="0.001" value={editingTensioningSpec.size} onChange={(e) => setTensioningData(prev => prev.map(t => t === editingTensioningSpec ? {...t, size: parseFloat(e.target.value) || 0} : t))} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-sm" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">B_ten: Min tensioning spacing (inch)</label>
                <input type="number" step="0.001" value={editingTensioningSpec.B_ten} onChange={(e) => setTensioningData(prev => prev.map(t => t === editingTensioningSpec ? {...t, B_ten: parseFloat(e.target.value) || 0} : t))} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-sm" />
              </div>
            </div>
